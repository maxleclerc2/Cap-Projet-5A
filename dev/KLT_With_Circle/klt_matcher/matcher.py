...
def main_KLT2(img_file, ref_file, mask_file, csv_file, xStart=0, N=6000, outliers=True):
    print("KLT...")
    # repo
    ...
    for xOff in range(0, img.xSize, N):
        ...
        for yOff in range(0, img.ySize, N):
            ...
            allPoints,points, Ninit = KLT_Tracker(ref_box, img_box, mask_box, maxCorners=maxCorners, matching_winsize=25,
                                        outliers=outliers)
...
def main_plot2(csv_file,allPoints, img_file, ref_file, outfile, xStart=0, factor=2):
    ...
    lim = 1
    lim_nb_kp = 500
    fig, axes = plt.subplots(2, 6, figsize=(15 * factor, 6 * factor), constrained_layout=True)

    for i, param in enumerate(["dx", 'dy']):
        ax = axes[i, 1]
        ax.set_title(f"{param} deviations (pixels)")
        ...
        sc = ax.scatter(points["x0"], points["y0"], c=colors, cmap=plt.cm.viridis, vmin=-lim, vmax=lim, s=1)
    ...
    # Sort the points by density, so that the densest points are plotted last
    ax = axes[0, 4]
    ax.set_title("scatter plot of the detected deviations dx, dy (pixels)")
    ax.set_xlabel("dx")
    ax.set_ylabel("dy")
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
    ...
    ############ circle plot with klt_img  on succestible coordinates of satellites ############
    # plot ref
    ax = axes[1, 0]
...
def plot_p(ax, img, ref_file, ref, p0):
...
def match(mon, ref, mask=None):
    # prefix
    mon_name = os.path.splitext(os.path.basename(mon))[0]
    ref_name = os.path.splitext(os.path.basename(ref))[0]
    csv_file = f'results/KLT_matcher_{mon_name}_{ref_name}.csv'
    png_file = csv_file.replace('.csv', '.png')

    # run matcher
    allPoints = main_KLT2(mon, ref, mask, csv_file, xStart=0, N=6000, outliers=True)

    # plot
    main_plot2(csv_file,allPoints, mon, ref, png_file)
