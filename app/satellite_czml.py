...
def get_date_from_tle(TLE):
    """
    Used to get the date from the TLE
    ...
    """
    month, day, hour, minute, second = days2mdhms(TLE.epochyr, TLE.epochdays)
    ...
    try:
        ...
        SEC = 00
        MILI = 0000
    date = datetime(TLE.epochyr, month, day, hour, minute, SEC, MILI)
    return pytz.utc.localize(date)


def sub_threading_func_pos(time_step, start_time, tle_object):
    """
    ...
    teme_position, tmp = tle_object.propagate(current_time.year, current_time.month,
                                           current_time.day, current_time.hour,
                                           current_time.minute, current_time.second)
    
    pos.append(time_step)
    pos.append(teme_position[0] * 1000)  # converts km's to m's
    pos.append(teme_position[1] * 1000)
    pos.append(teme_position[2] * 1000)

    return pos


def threading_func_pos(i, step, TLES):
    """
    UNUSED, PROBABLY TO REMOVE
    ...
    start_time = get_date_from_tle(TLES[i])
    end_time = get_date_from_tle(TLES[i + 1])
    
    """
    month, day, hour, minute, second = days2mdhms(TLES[i].epochyr, TLES[i].epochdays)
    # Separating seconds and milliseconds
    try:
        p = str(second)
        SEC = int(p.partition(".")[0])
        ...
        SEC = 00
        MILI = 0000
    date = datetime(TLES[i].epochyr, month, day, hour, minute, SEC, MILI)
    start_time = pytz.utc.localize(date)

    month, day, hour, minute, second = days2mdhms(TLES[i + 1].epochyr, TLES[i + 1].epochdays)
    # Separating seconds and milliseconds
    try:
        p = str(second)
        SEC = int(p.partition(".")[0])
        ...
        SEC = 00
        MILI = 0000
    date = datetime(TLES[i + 1].epochyr, month, day, hour, minute, SEC, MILI)
    end_time = pytz.utc.localize(date)
    """
    ...
    for _ in range(number_of_positions):
        time_step += step
        current_time = start_time + timedelta(seconds=time_step)
        eci_position, tmp = TLES[i].propagate(current_time.year, current_time.month,
                                              current_time.day, current_time.hour,
                                              current_time.minute, current_time.second)
...
def threading_func_path(i, sat, TLES):
    """
    UNUSED, PROBABLY TO REMOVE
    ...
    try:
        ...
        SEC = 00
        MILI = 0000
    start_time = datetime(TLES[i].epochyr, month, day, hour, minute, SEC, MILI)
    ...
    try:
        ...
        SEC = 00
        MILI = 0000
    end_time = datetime(TLES[i + 1].epochyr, month, day, hour, minute, SEC, MILI)
    
    # Path
    return sat.build_lead_trail_times(start_time, end_time)
...
class satellite():
    ...
    def __init__(self, tle, name=None, description=None, color=None, image=None,
                 ...
                 show_label=True, show_path=True, L_1=None, L_2=None):
        ...
        self.tle_obj = twoline2rv(self.L_1[0], self.L_2[0], wgs72)
        
        #print(self.L_1[0] + " " + self.L_2[0])
        self.TLES = []
        for i in range(len(L_1)):
            self.TLES.append(twoline2rv(self.L_1[i], self.L_2[i], wgs72))
    ...
    def build_path(self,
        ...
        if self.czmlPath is None or rebuild:
            ...
            if lead_times is None and trail_times is None:
                ...
                trail_times = []
                I = []
                for i in range(len(self.TLES) - 1):
                    I.append(i)
                
                pool = Pool(processes=cpu_count())
                results = pool.map(partial(threading_func_path, sat=self, TLES=self.TLES), I)

                for x in results:
                    lead_times.extend(x[0])
                    trail_times.extend(x[1])
                """
    ...
    def build_position(self,
        ...
        if self.czmlPosition is None:
            ...
            # Positions before the first TLE from 00:00.00 of the first day
            end_time_tmp = get_date_from_tle(self.TLES[0])

            number_of_positions = int((end_time_tmp - start_time).total_seconds() / 300)
            time_step = 0
            ...
            pool = Pool(processes=cpu_count())
            results = pool.map(partial(sub_threading_func_pos, start_time=start_time, tle_object=self.TLES[0]),
                               time_step_list)
            ...
            for _ in range(number_of_positions):
                time_step += step
                current_time = start_time + timedelta(seconds=time_step)
                teme_position, tmp = self.TLES[0].propagate(current_time.year, current_time.month,
                                                            current_time.day, current_time.hour,
                                                            current_time.minute, current_time.second)
            
                #eci_position, tmp = TEME2ECI(teme_position, tmp)
            
                positions.append(time_step)
                positions.append(teme_position[0] * 1000)  # converts km's to m's
                positions.append(teme_position[1] * 1000)
                positions.append(teme_position[2] * 1000)
            
            for i in I:
                start_time_tmp = get_date_from_tle(self.TLES[i])
                end_time_tmp = get_date_from_tle(self.TLES[i + 1])
    
                number_of_positions = int((end_time_tmp - start_time_tmp).total_seconds() / 300)
                ...
                pool = Pool(processes=cpu_count())
                results = pool.map(partial(sub_threading_func_pos, start_time=start_time, tle_object=self.TLES[i]),
                                   time_step_list)
                ...
                for _ in range(number_of_positions):
                    time_step += step
                    current_time = start_time + timedelta(seconds=time_step)
                    teme_position, tmp = self.TLES[i].propagate(current_time.year, current_time.month,
                                                                current_time.day, current_time.hour,
                                                                current_time.minute, current_time.second)
            ...
            # TODO Multiprocessing each TLE
            #pool = Pool(processes=cpu_count())
            #results = pool.map(partial(threading_func_pos, step=step, TLES=self.TLES), I)

            #for x in results:
            #   positions.extend(x)
            
            # Positions after the last TLE to 00:00.00 of the last day
            start_time_tmp = get_date_from_tle(self.TLES[-1])
            
            number_of_positions = int((end_time - start_time_tmp).total_seconds() / 300)
            ...
            pool = Pool(processes=cpu_count())
            results = pool.map(partial(sub_threading_func_pos, start_time=start_time, tle_object=self.TLES[-1]),
                               time_step_list)
            ...
            for _ in range(number_of_positions):
                time_step += step
                current_time = start_time + timedelta(seconds=time_step)
                teme_position, tmp = self.TLES[-1].propagate(current_time.year, current_time.month,
                                                             current_time.day, current_time.hour,
                                                             current_time.minute, current_time.second)
...
class satellite_czml():
    ...
    def get_czml(self):
        ...
        for id, sat in self.satellites.items():
            ...
            t = Thread(target=threading_func_sat, args=(id, sat,))
            t.start()
            threads.append(t)
